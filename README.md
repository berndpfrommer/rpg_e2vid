...
- install Nvidia 580 driver + Cuda 13
- create virtualenv (mkvirtualenv firenet or manually)
- install [pytorch](https://pytorch.org/get-started/locally/).
  Don't use the pytorch Cuda 13 version, but rather the Cuda 12.6 version because
  pytorch does not support the  older 4070Ti card.
  The magic line generated by the website was:
  ``pip3 install torch torchvision --index-url https://download.pytorch.org/whl/cu126``
- Install additional stuff:
  ``pip3 install PyYAML pandas opencv-python scipy``
- Get the ``use_latest_pybind11`` branch from the special
  [pybind11\_vendor package](https://github.com/ros-event-camera/pybind11_vendor) and the other
  packages below and build:
  ```
  mycolcon --packages-select pybind11_vendor event_camera_msgs event_camera_codecs event_camera_py
  source ./install/setup.bash
  ```
- Get the repo:
  ```
  git clone  git@github.com:berndpfrommer/rpg_e2vid.git
  git checkout firenet_from_rosbag
  ```
### Running the reconstruction

```
python3 ./run_reconstruction.py -i <input_bag> --topic <event_topic> -c ./firenet_1000.pth.tar --fixed_duration -T 25 -o <output_dir> --output_bag <output_bag>
```

Download here the pretrained [FireNet model](https://drive.google.com/file/d/1nBCeIF_Us-rGhCjdU5q1Ch-yrFckjZPa/view?usp=sharing) or pretrained E2VID model:
